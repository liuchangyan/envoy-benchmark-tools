...
do
    ...
    for((j=1; j<=3; j++))
    do
	 taskset -c 60-70  ../bazel-bin/nighthawk_client --rps $rps  --connections 64  --concurrency 4 --request-body-size 2048  --prefetch-connections -v info http://127.0.0.1:10000/  --duration 30  | grep -E  "0.75  |0.8  " | head -2 | awk '{print $3*1000000+$4*1000+$5}' | tr -d "a-zA-Z"   >>$rps.log
    done
